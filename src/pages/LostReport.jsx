...
function LostReport() {
    ...
    const [itemType, setItemType] = useState('electronic');
    ...
    async function sendData(e) {
        ...
        formData.append("uid", currUserUid);
        ...
        try{
            const response = await fetch('http://localhost:9000/product/upload', {
                method: 'POST',
                body: formData,
            });
    ...
    return (
        <div class="flex items-center justify-center min-h-screen lostReport">
            <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-sm">
                ...
                <form method='post' onSubmit={sendData}>
                    <div class="mb-4">
                        <input class="w-full px-3 py-2 border rounded-lg" placeholder="title" type="text" onChange={(e) => setTitle(e.target.value)} />
                    </div>
                    <div class="mb-4">
                        <input class="w-full px-3 py-2 border rounded-lg" placeholder="location" type="text" onChange={(e) => setLocation(e.target.value)} />
                    </div>
                    <div class="mb-4">
                        <textarea class="w-full px-3 py-2 border rounded-lg" id="desc" name="desc" placeholder="Describe here..." onChange={(e) => setDesc(e.target.value)}>
                        </textarea>
                    </div>
                    <div class="mb-4">
                        <select class="w-full px-3 py-2 border rounded-lg" id="category" name="category" onChange={(e) => setItemType(e.target.value)} required>
                            <option value="electronic">Electronic</option>
                            <option value="books">Book</option>
                            <option value="personalItems">Personal Item</option>
                    ...
                    <div class="mb-4">
                        <input class="w-full px-3 py-2 border rounded-lg" type="file" name="file" onChange={(e)=> setFile(e.target.files[0])}/>
                    </div>
                    <button class="w-full bg-black text-white py-2 rounded-lg mb-4" type='submit'>
                        Report
                    </button>
                </form>
            </div>
